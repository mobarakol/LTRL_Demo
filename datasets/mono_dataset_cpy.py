...
class MonoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(line) == 3:
            ...
            file_min = self.get_image_path(folder, frame_index+self.frame_idx_min, side)
            file_max = self.get_image_path(folder, frame_index+self.frame_idx_max, side)
            if not os.path.exists(file_min):
                frame_index += 1
            elif not os.path.exists(file_max):
                frame_index -= 1
        else:
            frame_index = 0
